...
export async function GET(request: Request) {
  ...
  return Response.json({ store: await readStore(), serverTime: Date.now() });
}
...
export async function POST(request: Request) {
  ...
  if (actionType === "round" || b.type === "round") {
    if (b.action === "PREPARE" && s.status === "WAITING") s.status = "READY";
      ...
      s.status = "LIVE";
      s.startedAt = now;
      s.endsAt = now + s.duration * 1000;
      s.pausedRemaining = s.duration;
      s.participants = s.participants.map((p) => (p.status === "VERIFIED" || p.status === "REGISTERED" ? { ...p, status: "ACTIVE" } : p));
    } else if (b.action === "PAUSE" && s.status === "LIVE") {
      s.pausedRemaining = s.endsAt ? Math.max(0, Math.ceil((s.endsAt - now) / 1000)) : s.pausedRemaining;
      s.endsAt = null;
      s.status = "PAUSED";
    } else if (b.action === "RESUME" && s.status === "PAUSED") {
      s.status = "LIVE";
      s.endsAt = now + s.pausedRemaining * 1000;
    } else if (b.action === "EXTEND" && ["LIVE", "PAUSED"].includes(s.status)) {
      s.pausedRemaining += 300;
      if (s.endsAt) s.endsAt += 300000;
    } else if (b.action === "END" && s.status !== "ENDED") {
      s.status = "ENDED";
      s.endsAt = null;
      s.pausedRemaining = 0;
      s.participants = s.participants.map((p) => (p.status !== "SUBMITTED" ? { ...p, status: "EXPIRED" } : p));
    } else if (b.action === "RESET") {
      s.status = "WAITING";
      s.startedAt = undefined;
      s.endsAt = null;
      s.pausedRemaining = s.duration || 1800;
    ...
    if (idx >= 0) {
      s.participants[idx] = { ...s.participants[idx], ...p };
    } else {
      s.participants.push({
        code: p.code,
        name: p.name || `Participant ${p.code}`,
        college: p.college || "General",
        challenge: p.challenge || (s.challenges.length > 0 ? s.challenges[s.participants.length % s.challenges.length].code : ""),
